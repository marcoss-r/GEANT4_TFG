...
"pistola" que lanzará nuestras partículas. Creamos la pistola, definimos qué
partícula usamos, el vector velocidad, su energía y la posición inicial */
...
PrimaryGenAction::PrimaryGenAction(){
    ...
    if(!fIsLoaded){
        ...
        LoadParticlesFromCSV("/home/geant/Desktop/Marcos_TFG_GEANT/"
            "git/GEANT4_TFG/aplicacion6/input/input.csv");
        fIsLoaded = true;
        /* Debugging */
        G4cout << "Loaded " << fParticleList.size()
               << " particles from CSV" << G4endl;
    }
...
void PrimaryGenAction::GeneratePrimaries(G4Event* anEvent){
    ...
    fparticleGun->SetParticlePosition(G4ThreeVector(
            particle.pos_x * m, particle.pos_y * m, particle.pos_z * m));

    /* Dirección */
    fparticleGun->SetParticleMomentumDirection(G4ThreeVector(
            particle.dir_x, particle.dir_y, particle.dir_z));
